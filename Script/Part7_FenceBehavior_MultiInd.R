...
foreach (i = unique(movement.df.all$Location.ID)) %dopar% { #animal ID, in this dataframe is named as Location ID. 
  ...
  event.df.i <- data.frame(AnimalID = character(l),  burstID = numeric(l), easting = numeric(l), northing = numeric(l), duration = numeric(l), straightness = numeric(l), eventTYPE = character(l), stringsAsFactors=FALSE)
